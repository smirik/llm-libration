...
@click.option('--output-file', type=click.Path(path_type=Path), help='Output PNG file path (default: same as input with .png extension)')
...
def plot(input_file: Path, x_column: str, y_column: str, output_file: Optional[Path], y_min: float, y_max: float):
    """Create a plot from CSV data.

    INPUT_FILE: Path to the CSV file containing the data to plot.
    """
    try:
        result_path = create_plot(
            input_file=input_file, x_column=x_column, y_column=y_column, output_file=output_file, y_min=y_min, y_max=y_max
        )
        click.echo(f"Plot created successfully: {result_path}")
